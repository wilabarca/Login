...
void main() {
  ...
  setUp(() {
    ...
    remoteWipeService = RemoteWipeService.createWithStorage(
  ...
  RemoteMessage createMessage({
    ...
    return RemoteMessage(
      data: {
        ...
        'expiresAt': now
            .add(Duration(seconds: expiresOffset))
            .toIso8601String(),
      },
    );
  }
  ...
  group('RemoteWipeService Validations', () {
    test(
      '1. Orden válida para admin elimina los cuatro campos y retorna applied',
      () async {
    ...
    test(
      '2. Usuario incorrecto no elimina ningún campo y retorna wrongUser',
      () async {
        await secureStorageService.seedSensitiveData(remoteUserId: 'admin');
        ...
        for (final isSaved in status.values) {
          expect(
            isSaved,
            true,
            reason: 'El campo fue eliminado pero no debió serlo',
          );
        }
      },
    );
    ...
    test(
      '4. Acción incorrecta no elimina datos y retorna invalidAction',
      () async {
        await secureStorageService.seedSensitiveData(remoteUserId: 'admin');
    ...
    test(
      '5. Payload incompleto no elimina datos y retorna invalidPayload',
      () async {
        await secureStorageService.seedSensitiveData(remoteUserId: 'admin');

        final msg = RemoteMessage(
          data: {'action': 'remote_wipe'},
        ); // Sin ID, targetUserId, fechas
        final result = await remoteWipeService.handleRemoteMessage(msg);
    ...
    test(
      '6. commandId duplicado no elimina nuevamente y retorna duplicate',
      () async {
        await secureStorageService.seedSensitiveData(remoteUserId: 'admin');
        ...
        await secureStorageService.seedSensitiveData(
          remoteUserId: 'admin',
        ); // Simulamos re-ingreso
    ...
    test(
      '9. Después de applied, los cuatro campos sensibles están ausentes',
      () async {
        await secureStorageService.seedSensitiveData(remoteUserId: 'admin');
    ...
    test(
      '10. target_user_id e installationId permanecen después del borrado',
      () async {
        await secureStorageService.seedSensitiveData(remoteUserId: 'admin');
